import argparse
from trt_utils.segcolors import colors, lanecolor, midcolor
import numpy as np
...
if __name__ == '__main__':
	...
	# calibrator=Calibrator('./val2017/', 1000)
	calibrator = None
	...
	seg=TRTSegmentor('./checkpoints_trt/bisenet110_360.onnx', colors, 
		device=device, 
		precision=precision,
		calibrator=calibrator, 
		dla_core=dla_core)
	cap = cv2.VideoCapture('filesrc location=./video/project2.avi ! qtdemux ! queue ! h264parse ! omxh264dec ! nvvidconv ! video/x-raw,format=BGRx ! queue ! videoconvert ! queue ! video/x-raw, format=BGR ! appsink', cv2.CAP_GSTREAMER)
	seg.infervideo(cap)
